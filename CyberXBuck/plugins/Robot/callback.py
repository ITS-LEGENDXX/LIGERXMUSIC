...
from CyberXBuck import YouTube, app
from CyberXBuck.core.call import Cyber
from CyberXBuck.misc import SUDOERS, db
from CyberXBuck.utils import bot_sys_stats
from CyberXBuck.utils.database import (is_active_chat,
...
from CyberXBuck.utils.formatters import seconds_to_min
from CyberXBuck.utils.inline.play import (panel_markup_1,
                                          stream_markup,
                                          telegram_markup,
                                          close_keyboard)
from CyberXBuck.utils.stream.autoclear import auto_clean
from CyberXBuck.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Cyber.pause_stream(chat_id)
        ...
        await Cyber.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await CallbackQuery.answer()
        await Cyber.stop_stream(chat_id)
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Cyber.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Cyber.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await Cyber.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
